...
public class ProcedureProvider extends ContentProvider {
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        case PROCEDURES:    
            break;
        case PROCEDURE_ID:
            qb.appendWhere(ProcedureSQLFormat._ID + "=" + uri.getPathSegments().get(1));
            break;
        default:
    ...
    public int update(Uri uri, ContentValues values, String selection,
        ...
        switch(sUriMatcher.match(uri)) {
        case PROCEDURES:
            count = db.update(PROCEDURE_TABLE_NAME, values, selection, selectionArgs);
            break;
            
        case PROCEDURE_ID:
            String procedureId = uri.getPathSegments().get(1);
            count = db.update(PROCEDURE_TABLE_NAME, values, ProcedureSQLFormat._ID + "=" + procedureId + (!TextUtils.isEmpty(selection) ? " AND (" + selection + ")" : ""), selectionArgs);
            break;
        default:
            throw new IllegalArgumentException("Unknown URI " + uri);
        }
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        case PROCEDURE_ID:
            String procedureId = uri.getPathSegments().get(1); 
            count = db.delete(PROCEDURE_TABLE_NAME, ProcedureSQLFormat._ID + "=" + procedureId + (!TextUtils.isEmpty(selection) ? " AND (" + selection + ")" : ""), selectionArgs);
            break;
        default:
            throw new IllegalArgumentException("Unknown URI " + uri);
        }
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
        ...
        if(values.containsKey(ProcedureSQLFormat.TITLE) == false) {
            ...
            values.put(ProcedureSQLFormat.TITLE, r.getString(android.R.string.untitled));
        }
        
        if(values.containsKey(ProcedureSQLFormat.AUTHOR) == false) {
            Resources r = Resources.getSystem();
            values.put(ProcedureSQLFormat.AUTHOR, r.getString(android.R.string.untitled));
        }
        ...
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        long rowId = db.insert(PROCEDURE_TABLE_NAME, ProcedureSQLFormat.PROCEDURE, values);
        if(rowId > 0) {
            Uri noteUri = ContentUris.withAppendedId(ProcedureSQLFormat.CONTENT_URI, rowId);
    ...
    static {
        ...
        sUriMatcher.addURI(MocaDB.PROCEDURE_AUTHORITY, "procedures", PROCEDURES);
        sUriMatcher.addURI(MocaDB.PROCEDURE_AUTHORITY, "procedures/#", PROCEDURE_ID);

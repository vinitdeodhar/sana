...
public class NotificationProvider extends ContentProvider {
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        case NOTIFICATION_ID:
            qb.appendWhere(NotificationSQLFormat._ID + "=" + uri.getPathSegments().get(1));
            break;
        default:
    ...
    public int update(Uri uri, ContentValues values, String selection,
        ...
        switch(sUriMatcher.match(uri)) {
        case NOTIFICATIONS:
            count = db.update(NOTIFICATION_TABLE_NAME, values, selection, selectionArgs);
            break;
            
        case NOTIFICATION_ID:
            String procedureId = uri.getPathSegments().get(1);
            count = db.update(NOTIFICATION_TABLE_NAME, values, NotificationSQLFormat._ID + "=" + procedureId + (!TextUtils.isEmpty(selection) ? " AND (" + selection + ")" : ""), selectionArgs);
            break;
        default:
            throw new IllegalArgumentException("Unknown URI " + uri);
        }
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        case NOTIFICATION_ID:
            ...
            count = db.delete(NOTIFICATION_TABLE_NAME, NotificationSQLFormat._ID + "=" + imageId + (!TextUtils.isEmpty(selection) ? " AND (" + selection + ")" : ""), selectionArgs);
            break;
        default:
            throw new IllegalArgumentException("Unknown URI " + uri);
        }
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
        ...
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        long rowId = db.insert(NOTIFICATION_TABLE_NAME, NotificationSQLFormat.PROCEDURE_ID, values);
        if(rowId > 0) {
            Uri notificationUri = ContentUris.withAppendedId(NotificationSQLFormat.CONTENT_URI, rowId);
    ...
    static {
        ...
        sUriMatcher.addURI(MocaDB.NOTIFICATION_AUTHORITY, "notifications", NOTIFICATIONS);
        sUriMatcher.addURI(MocaDB.NOTIFICATION_AUTHORITY, "notifications/#", NOTIFICATION_ID);

...
public class SavedProcedureProvider extends ContentProvider {
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        case SAVED_PROCEDURE_ID:
            qb.appendWhere(SavedProcedureSQLFormat._ID + "=" + uri.getPathSegments().get(1));
            break;
        default:
    ...
    public int update(Uri uri, ContentValues values, String selection,
        ...
        case SAVED_PROCEDURE_ID:
            ...
            count = db.update(SAVED_PROCEDURE_TABLE_NAME, values, SavedProcedureSQLFormat._ID + "=" + procedureId + (!TextUtils.isEmpty(selection) ? " AND (" + selection + ")" : ""), selectionArgs);
            break;
        default:
            throw new IllegalArgumentException("Unknown URI " + uri);
        }
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        case SAVED_PROCEDURES:
        	...
        	Cursor c = query(SavedProcedureSQLFormat.CONTENT_URI, new String[] { SavedProcedureSQLFormat._ID }, selection, selectionArgs, null);
        	ArrayList<String> idList = new ArrayList<String>(c.getCount());
        	if(c.moveToFirst()) {
        		while(!c.isAfterLast()) {
        			String id = c.getString(c.getColumnIndex(SavedProcedureSQLFormat._ID));
        			idList.add(id);
        			c.moveToNext();
        		}
        	}
        ...
        case SAVED_PROCEDURE_ID:
        	String procedureId = uri.getPathSegments().get(1);
            count = db.delete(SAVED_PROCEDURE_TABLE_NAME, SavedProcedureSQLFormat._ID + "=" + procedureId + (!TextUtils.isEmpty(selection) ? " AND (" + selection + ")" : ""), selectionArgs);
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
        ...
        if(values.containsKey(SavedProcedureSQLFormat.GUID) == false) {
        	values.put(SavedProcedureSQLFormat.GUID, MocaUtil.randomString("SP", 20));
        }
        ...
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        long rowId = db.insert(SAVED_PROCEDURE_TABLE_NAME, SavedProcedureSQLFormat.PROCEDURE_STATE, values);
        if(rowId > 0) {
            Uri savedProcedureUri = ContentUris.withAppendedId(SavedProcedureSQLFormat.CONTENT_URI, rowId);
            getContext().getContentResolver().notifyChange(savedProcedureUri, null);
